...
class Sale:
    ...
    def get_headings(self):
        return ['processTime', 'reseller'] + [i for i in self._keys]

    def get_data(self):
        return [self.processTime, self.reseller] + self._values
...
class SaleSTUB(Sale):
    def __init__(self):
        ...
        # self.map = dict(zip(self._keys, [list(a) for a in zip(self.tags, self.offsets, self.split_string, self.split_element, values)]))
